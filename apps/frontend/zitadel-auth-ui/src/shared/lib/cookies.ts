...
import { timestampDate, timestampFromMs } from "@task-manager/zitadel-client";
...
export async function addSessionToCookie<T>(session: SessionCookie<T>, cleanup = false) {
	const cookiesList = await cookies();
...
export async function updateSessionCookie<T>(id: string, session: SessionCookie<T>, cleanup = false) {
	const cookiesList = await cookies();
	const stringifiedCookie = cookiesList.get("sessions");
...
export async function removeSessionFromCookie<T>(session: SessionCookie<T>, cleanup = false) {
	const cookiesList = await cookies();
	const stringifiedCookie = cookiesList.get("sessions");
 ...
 */
export async function getAllSessionCookieIds<T>(cleanup = false) {
	const cookiesList = await cookies();
	const stringifiedCookie = cookiesList.get("sessions");
 ...
 * @returns Session Cookies
 */
export async function getAllSessions<T>(cleanup = false): Promise<SessionCookie<T>[]> {
	const cookiesList = await cookies();
	const stringifiedCookie = cookiesList.get("sessions");
