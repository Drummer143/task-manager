...
import { PasskeysType } from "@task-manager/zitadel-api/zitadel/settings/v2/login_settings_pb";
import { UserState } from "@task-manager/zitadel-api/zitadel/user/v2/user_pb";
...
export async function sendLoginname(command: SendLoginnameCommand) {
	...
	if (potentialUsers.length > 1) {
		...
		if (!methods.authMethodTypes || !methods.authMethodTypes.length) {
			...
			const inviteCheck = checkInvite(session, humanUser, session.factors.user.organizationId, command.requestId);
		...
		if (methods.authMethodTypes.length === 1) {
			...
			switch (method) {
				case AuthenticationMethodType.PASSWORD: {
					// user has only password as auth method
