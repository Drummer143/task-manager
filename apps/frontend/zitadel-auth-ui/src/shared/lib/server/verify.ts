...
import { User } from "@task-manager/zitadel-api/zitadel/user/v2/user_pb";
...
export type SendVerificationRedirectWithoutCheckCommand = {
	organization?: string;
	requestId?: string;
} & ({ userId: string; loginName?: never } | { userId?: never; loginName: string });

export async function sendVerificationRedirectWithoutCheck(command: SendVerificationRedirectWithoutCheckCommand) {
	const _headers = await headers();
	const { serviceUrl } = getServiceUrlFromHeaders(_headers);
