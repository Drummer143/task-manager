...
import { Duration } from "@task-manager/zitadel-client";
...
import { deleteSession, getLoginSettings, humanMFAInitSkipped, listAuthenticationMethodTypes } from "../zitadel";
...
export async function updateSession(options: UpdateSessionCommand) {
	...
	const session = await setSessionAndUpdateCookie(recentSession, checks, challenges, requestId, lifetime);
