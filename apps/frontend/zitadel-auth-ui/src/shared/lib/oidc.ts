import { CreateCallbackRequestSchema, SessionSchema } from "@task-manager/zitadel-api/zitadel/oidc/v2/oidc_service_pb";
...
import { createCallback, getLoginSettings } from "./zitadel";
			...
			try {
				...
				if (error && typeof error === "object" && "code" in error && error?.code === 9) {
					...
					if (selectedSession.factors?.user?.loginName) {
						signedinUrl.searchParams.set("loginName", selectedSession.factors?.user?.loginName);
					}
