...
#[utoipa::path(
    ...
    responses(
        (status = 200, description = "Status retrieved successfully", body = FullBoardStatusResponseDto),
...
pub async fn get_board_status_by_id(
    ...
    crate::entities::board_statuses::service::get_board_status_by_id(&app_state.postgres, status_id)
        .await
        .map(|status| {
