...
use crate::{
    controllers::page::dto::{PageAccessResponse, UpdatePageAccessRequest},
...
#[utoipa::path(
    ...
    responses(
        (status = 200, description = "Page access updated successfully", body = crate::controllers::page::dto::PageAccessResponse),
...
pub async fn update_page_access(
    ...
    let page_access = PageService::update_page_access(
        &state.postgres,
        UpdatePageAccessDto {
            user_id: dto.user_id,
    ...
    Ok(axum::Json(PageAccessResponse {
        id: page_access.id,
        user: UserService::get_one_by_id(&state.postgres, page_access.user_id).await?,
